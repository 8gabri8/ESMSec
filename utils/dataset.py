...
def create_dataloader(processed_df, set_name, batch_size, shuffle=False, pin_memory=True):
    ...
    # Stack tensors
    input_ids = torch.stack(processed_df.loc[set_idx, 'input_ids'].tolist())
    attention_mask = torch.stack(processed_df.loc[set_idx, 'attention_mask'].tolist())
    labels = torch.tensor(processed_df.loc[set_idx, 'label'].values)
